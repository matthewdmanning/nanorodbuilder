...
for thick, radius, curve, density, ligand, ratio in itertools.product(thicklist, radiuslist, curvelist, densitylist,
                                                                      ligandlist, ratiolist):
    try:
        ...
        DUnum = 0
        print
        'The size of the renamed list is {}.\n'.format(len(newnamelist))
        ...
        print
        '{} atoms reassigned as from DU to hc.'.format(DUnum)
        RenamedFile.close()
        OutputFile.close()
        InputFile.close()
    except IOError:
