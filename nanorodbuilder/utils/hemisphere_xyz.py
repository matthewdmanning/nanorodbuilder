...
if __name__ == '__main__':
    ...
    for atom_index, atom_coords in enumerate(sphere_coords_array):
        ...
        if radius - np.sqrt(atom_coords[0] ** 2 + atom_coords[1] ** 2 + atom_coords[2] ** 2) < 1.0 and abs(
                atom_coords[0] - atom_coords[1]) - 2.0 < 0.0001 and abs(atom_coords[1] - atom_coords[2]) - 2.0 < 0.0001:
    ...
    leapshell.write(
        'tail = loadpdbusingseq original_original-last-new.pdb {NSER GLY ARG GLY LYS GLY GLY LYS GLY LEU GLY LYS GLY GLY ALA LYS ARG HIP ARG LYS VAL LEU ARG ASP ASN ILE}\n')
