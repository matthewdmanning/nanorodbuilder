...
def move_ligands(leap_system, nanoparticle, coords_multiplier=1000., throw=4., rotate_lig=False,
                 ligand_vector=np.array([0, 0, -1.])):
    for site, ligand in nanoparticle.site_ligand_dict.items():
        ...
        if rotate_lig:
            ...
            leap_system.input_file.write(
                'transform {0} {{ {{ {1[0][0]} {1[0][1]} {1[0][2]} }} {{ {1[1][0]} {1[1][1]} {1[1][2]} }} {{ {1[2][0]} {1[2][1]} {1[2][2]} }} }}\n'.format(
                    ligand.amber_alias, rotation_vector))
            # leap_system.input_file.write('translate {0} {{ 0 0 -30 }}\n'.format(ligand.amber_alias))
