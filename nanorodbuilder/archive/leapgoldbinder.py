        ...
        for i in range(counter + 1, atomsize - 1):
            ...
            if math.sqrt((atom[1] - checkatom[1]) ** 2 + (atom[2] - checkatom[2]) ** 2 + (
                ...
                if int(atom[0]) in goldid and int(checkatom[0]) not in goldid:
                    ...
                    print
                    'Overlapping ligand found. Deleting ligand residue: gold.{}.\n'.format(
                    ...
                    # ## Both atoms are bound to ligands. -> Not good. Choose one gold/ligand and delete.
                        ...
                        print
                        'Deleting residue gold.{} and Au gold.{}.{}'.format(goldsulflig[row, 2],
                                                                            residuelist[int(checkatom[0])],
                                                                            namelist[int(checkatom[0])])
                        atomsdelete.append(i)
                        ausurvivors.append(counter)
...
def leap_script(filepath, mol2_name, goldfilename, leapshellname):
    ...
    for line in goldfile:
        ...
        if trigger == 1:
            atomslist.append(
                [int(spline[0]) - 1, float(spline[2]), float(spline[3]), float(spline[4]), 0, 0, 0, 0, 0, 0, 0, 0, 0])
    ...
    success, atomsdelete, residuedelete, ausurvivors = overlap_check(atoms, goldid, protected, goldsulflig, residuelist,
                                                                     namelist, residuedelete)
    ...
    for counter in range(atomsize - 1):
        ...
        for i in range(counter + 1, atomsize - 1):
            ...
            checkatom = atoms[i]
            if int(checkatom[0]) in atomsdelete or int(checkatom[0]) in fullbond:
                continue
    ...
    leapshell.write(
        'source /home/mdmannin/amber16/dat/leap/cmd/oldff/leaprc.ff14SB \nloadamberparams /home/mdmannin/amber16/dat/leap/parm/AuNP.frcmod\n')
    leapshell.write(
        'loadamberparams /home/mdmannin/amber16/dat/leap/parm/frcmod.ionsjc_tip3p\nloadamberparams gaff.dat\n')
    ...
    for counter, atom in enumerate(atomslist):
        ...
        if counter in atomsdelete:
            leapshell.write('remove gold.{} gold.{}.{}\n'.format(residuelist[counter], residuelist[counter],
                                                                 namelist[int(atom[0])]))
            continue
        # ## Alias all gold atoms for reference after move to new residue.
    # Bond gold atoms to neighbors.
    for bonding in bondlist:
        leapshell.write('bond gold.{}.{} gold.{}.{}\n'.format(residuelist[bonding[0]], namelist[bonding[0]],
                                                              residuelist[bonding[1]], namelist[bonding[1]]))
...
def main():
    def master_file():
        ...
        for length, radius, density, ligand, ratio in itertools.product(lengthlist, radiuslist, densitylist, ligandlist,
                                                                        ratiolist):
    ...
    for length, radius, density, ligand, ratio in itertools.product(lengthlist, radiuslist, densitylist, ligandlist,
        ...
        parmpath = '{}ParmFiles/'.format(filepath)
        print
        'Input file: {}.mol2.'.format(goldfilename)
        print
