...
def bodycheck(x, y, z, bodylength, apothem, caplength, capangle):
    return (abs(y) + abs(z) - (1 + m.tan(m.pi / 8)) * apothem < 0.0001) and (abs(y) - apothem < 1) and (
                abs(z) - apothem < 1)


def corecheck(x, y, z, bodylength, apothem, caplength, capangle):
    return ((abs(y) + abs(z) - (1 + m.tan(m.pi / 8)) * apothem + surfthick < 0.00001) and (
                abs(y) - apothem + surfthick < 0.00001) and (abs(z) - apothem + surfthick < 0.0001)) and abs(x) - (
        bodylength) / 2 + caplength < 0.0001


def surfbondcheck(x, y, z, bodylength, apothem, caplength, capangle):
    return ((abs(y) + abs(z) - (1 + m.tan(m.pi / 8)) * apothem + surfthick > 0.0001) or (
                abs(y) - apothem > -surfthick) or (abs(z) - apothem > -surfthick)) and abs(x) - (
        bodylength) / 2 + caplength - surfthick < 0.0001
...
def capcheck(x, y, z, bodylength, apothem, caplength, capangle):
    ...
    return (x - bodylength / 2 + caplength > 0.0001) and (
                abs(y) - slopeheight < 0.0001 and abs(z) - slopeheight < 0.0001) and (
                (abs(y) - abs(z) - (1 + m.tan(m.pi / 8)) * apothem < 0.00001) or (
                    abs(y) + abs(z) - (1 + m.tan(m.pi / 8)) * apothem < 0.00001) and (abs(y) - apothem < 0.00001) and (
                            abs(z) - apothem < 0.0001))


def capbondcheck(x, y, z, bodylength, apothem, caplength, capangle):
    slopeheight = apothem - np.tan(capangle) * (x - bodylength / 2 + caplength)
    capthick = surfthick * 1.5
    return (z - slopeheight + capthick > 0.0001) or (
                y + z - (1 + m.tan(m.pi / 8)) * apothem + capthick > 0.0001 and z - y - (
                    ...
                    for i in range(0, xextender):
                        ...
                        if surfbondcheck(newx, newy, newz, length, apothem, caplength, capangle):
                            if (newx, newy, newz) not in shellarray:
                                ...
                                shellnum += 1
                            if surfbondnormalcheck(newx, newy, newz, length, apothem, caplength, capangle) and (
                            newx, newy, newz) not in normalarray and (newx, newy, newz) not in diagonalarray:
                                normalarray.append([newy, newx, newz])
                                normalnum += 1
                            elif surfbonddiagonalcheck(newx, newy, newz, length, apothem, caplength, capangle) and (
                            newx, newy, newz) not in diagonalarray and (newx, newy, newz) not in normalarray:
                                diagonalarray.append([newy, newx, newz])
                                diagonalnum += 1
                        elif corecheck(newx, newy, newz, length, apothem, caplength, capangle) and (
                        newx, newy, newz) not in corearray:
                            ...
                            if capbondcheck(newx, newy, newz, length, apothem, caplength, capangle) and (
...
def ligandpicker():
    ...
    for liganddensity in densitylist:
        ...
        print
        'Minimum ligand separation distance is {} Angstrom.'.format(cutoff)
        rotatedcoords[:] = []
        ...
        rotateout.close()
        print('Density = {}. {} ligands on {} ^2'.format(len(rotatedcoords) * 100 / surfacearea, len(rotatedcoords),
                                                         surfacearea))
